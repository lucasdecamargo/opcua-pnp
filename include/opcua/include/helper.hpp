...
#ifndef OPCUA_HELPER_H
#define OPCUA_HELPER_H
#pragma once
...
namespace pnp {
    namespace opcua {
        ...
        inline std::future<PROMISE_TYPE> setPromiseErrorException(
            ...
            try {
                throw pnp::opcua::StatusCodeException(code);
        ...
        inline const std::shared_ptr<UA_NodeId> UA_Server_getObjectChildId(
            ...
            if (retval != UA_STATUSCODE_GOOD || bpr.targetsSize < 1) {
                ...
                throw pnp::opcua::NodeNotFoundException(qualifiedName, errorMessage);
            }
        ...
        inline const std::shared_ptr<UA_NodeId> UA_Server_getObjectChildId(
                const std::shared_ptr<pnp::opcua::OpcUaServer>& uaServer,
                const UA_NodeId objectId,
                const UA_QualifiedName childName,
                const UA_NodeId reference
        ) {
        ...
        UA_Server_getObjectComponentId_or_Null(
            ...
            try {
                return UA_Server_getObjectChildId(server, objectId, componentName,
                                                  UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT));
            } catch (pnp::opcua::NodeNotFoundException& ex) {
                return NULL;
            }
        }
        ...
        inline bool UA_Client_findBrowsePath(
            ...
            UA_BrowsePath_clear(&browsePathReq);
        ...
        inline bool UA_Client_findChildWithNameRecursive(
                UA_Client* client,
                const std::shared_ptr<spdlog::logger>& logger,
                const UA_NodeId& parentNode,
                const UA_String& name,
            ...
            if (bResp.responseHeader.serviceResult != UA_STATUSCODE_GOOD) {
                ...
                for (size_t i = 0; i < bResp.resultsSize; ++i) {
                    for (size_t j = 0; j < bResp.results[i].referencesSize; ++j) {
                        ...
                        {
                            if(UA_Client_findChildWithNameRecursive(client, logger, ref->nodeId.nodeId, 
                                                                        name, foundNodeId, n-1))
                                found = true;
                        }
                    }
                }
        ...
        UA_Client* UA_Helper_getClientForEndpoint(
...
#ifdef UA_ENABLE_ENCRYPTION
                UA_ByteString certificate = pnp::opcua::loadBinaryFile(clientCertPath);
                UA_ByteString privateKey = pnp::opcua::loadBinaryFile(clientKeyPath);
            ...
            clientConfig->timeout = 2000;
            pnp::opcua::UA_ClientConfig_setUriName(clientConfig, clientAppUri.c_str(), clientAppName.c_str());
            ...
            if (hasUsernamePwd) {
                ...
                UA_ExtensionObject_clear(&clientConfig->userIdentityToken);
        ...
        initServerConfig(
            ...
            config->logger.log = pnp::log::opcua::UA_Log_Spdlog_log;
            config->logger.context = logger.get();
            config->logger.clear = pnp::log::opcua::UA_Log_Spdlog_clear;
            ...
            if (enableEncryption) {
                ...
                UA_ByteString certificate = pnp::opcua::loadBinaryFile(certFile);
                UA_ByteString privateKey = pnp::opcua::loadBinaryFile(keyFile);
            ...
            pnp::opcua::UA_ServerConfig_setUriName(config, appUri.c_str(),
                                                       appName.c_str());
        ...
        inline UA_StatusCode UA_Client_getStartTime(
            ...
            std::promise<std::shared_ptr<UA_EndpointDescription>> promiseEndpoint;
            pnp::opcua::getBestEndpointFromServer(discoveryUrls, promiseEndpoint, loggerClient);
