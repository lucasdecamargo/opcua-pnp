...
SkillClient::SkillClient(std::shared_ptr<spdlog::logger> _loggerApp,
    ...
    if (clientParam == NULL) {
        ...
        clientConfig->logger.log = pnp::log::opcua::UA_Log_Spdlog_log;
        clientConfig->logger.context = loggerOpcua.get();
        clientConfig->logger.clear = pnp::log::opcua::UA_Log_Spdlog_clear;
    } else {
        client = clientParam;
    }
...
void SkillClient::initNodes(bool isParameterSetMandatory) {
    ...
    bool found;
    found = pnp::opcua::UA_Client_findChildWithBrowseName(client, logger, skillNodeId,
    ...
    found = pnp::opcua::UA_Client_findChildWithBrowseName(client, logger, currentStateId,
    ...
    pnp::opcua::UA_Client_findChildrenWithReferenceType(client, logger, UA_NODEID_NUMERIC(0, UA_NS0ID_PROGRAMTRANSITIONEVENTTYPE),
                                                 UA_NODEID_NUMERIC(0,
                                                                   UA_NS0ID_HASSUBTYPE),
...
void SkillClient::handleTransitionEvent(const UA_DateTime *time,
    ...
    pnp::opcua::ProgramStateNumber newState;
    ...
    if (UA_String_equal(&toState->text, &readyState))
        newState = pnp::opcua::ProgramStateNumber::READY;
    else if (UA_String_equal(&toState->text, &haltedState))
        newState = pnp::opcua::ProgramStateNumber::HALTED;
    else if (UA_String_equal(&toState->text, &runningState))
        newState = pnp::opcua::ProgramStateNumber::RUNNING;
    else if (UA_String_equal(&toState->text, &suspendedState))
        newState = pnp::opcua::ProgramStateNumber::SUSPENDED;
...
SkillClient::resetWait() {
    ...
    if (retval != UA_STATUSCODE_GOOD) {
        return pnp::opcua::setPromiseErrorException<void>(&promiseWait, retval);
    }
    return std::async([this]() {
        pnp::opcua::ProgramStateNumber newState = getNextState().get();
...
const pnp::opcua::ProgramStateNumber SkillClient::getCurrentState() {
    ...
    pnp::opcua::ProgramStateNumber number = *(static_cast<pnp::opcua::ProgramStateNumber *>(var.data));

    UA_Variant_clear(&var);
    return number;
}

const pnp::opcua::ProgramTransitionNumber SkillClient::getLastTransition() {
    UA_ReadRequest request;
    UA_ReadRequest_init(&request);
    request.nodesToReadSize = 1;
    request.nodesToRead = UA_ReadValueId_new();
    ...
    pnp::opcua::ProgramTransitionNumber number = *(static_cast<pnp::opcua::ProgramTransitionNumber *>(response.results->value.data));
    UA_ReadResponse_clear(&response);
    return number;
}

void SkillClient::emptyReceivedStates() {
    std::queue<pnp::opcua::ProgramStateNumber>().swap(receivedStates);
}

std::future<pnp::opcua::ProgramStateNumber> SkillClient::getNextState() {
    if (receivedStates.size() == 0) {
        ...
        return std::async([this]() {
            ...
            if (receivedStates.size()) {
                pnp::opcua::ProgramStateNumber state = receivedStates.front();
            ...
            return pnp::opcua::ProgramStateNumber::INVALID;
        });
    }
    std::promise<pnp::opcua::ProgramStateNumber> promiseState;
...
void SkillClient::setEventFoundCallback(const std::function<bool(const pnp::opcua::ProgramStateNumber)> &callback) {
...
bool SkillClient::initParameter(SkillClientParameter** param, const std::string &name, const UA_QualifiedName &browseName) {
    ...
    {
        std::lock_guard<std::recursive_mutex> lk(clientMutex);
        found = pnp::opcua::UA_Client_findChildWithBrowseName(client, logger, parameterSetNodeId,
                                                                       browseName,
                                                                       foundId);
    }
    if (!found) {
...
std::future<UA_StatusCode> SkillClient::getFinalResultData(const UA_String& resultData, UA_Variant *data, bool disconnectAfter) {
    return std::async([this, resultData, data, disconnectAfter]() {
...

        {
            ...
            if (ret != UA_STATUSCODE_GOOD)
                return ret;
            found = pnp::opcua::UA_Client_findChildWithName(client, logger, finalResultDataNodeId,
                                                                      resultData,
                                                                      &finalResultDataId);
            if (disconnectAfter)
                this->disconnect();
