...
bool SkillDetector::findAllSkillControllerNodes(
    ...
    UA_NodeId skillControllerType = UA_NODEID_NUMERIC(component->nsFortissDiIdx, UA_DEVICE_MODELID_ISKILLCONTROLLERTYPE);

    std::vector<std::shared_ptr<UA_NodeId>> foundIds;

    {
        LockedClient lc = component->getLockedClient();
        if (!pnp::opcua::UA_Client_findChildrenImplementingInterfaceRecursively(lc.get(), logger, &deviceSetId, 1, skillControllerType, foundIds))
            return false;
    }
...
bool SkillDetector::addSkillsFromController(
    ...
    {
        LockedClient lc = component->getLockedClient();
        found = pnp::opcua::UA_Client_findChildWithBrowseName(lc.get(), logger, *controllerId,
...
std::shared_ptr<UA_EndpointDescription> SkillDetector::findAvailableEndpoint(
    ...
    pnp::opcua::getBestEndpointFromServer(discoveryUrls, promiseEndpoint, loggerOpcua);
...
bool SkillDetector::addServerKnown(
    ...
    if (startTime == 0) {
        UA_StatusCode retval = pnp::opcua::UA_Client_getStartTime(
                logger,
                loggerOpcua,
                clientCertPath,
                clientKeyPath,
